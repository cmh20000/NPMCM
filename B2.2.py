...
def SA(num):
    ...
    build_lp_data_static()
    build_target()
    build_constrain_static()
    ...
    build_remained_con(last_solution, num)
    lp_frame(False)

    # print("lp_result:", lp_result)

    last_value = lp_result
    best_value = lp_result
    ...
    while cur_t > min_t:
        ...
        build_remained_con(new_solution, num)  # 这个函数的唯一目的就是更新visit
        lp_frame(False)
        # print("lp_result:", lp_result)
    ...
    build_remained_con(best_solution, num, True)  # 这个函数的唯一目的就是更新visit
    lp_frame(True)
    best_lp_value = lp_result

    return best_solution

# 4、处理11个之后的连接
def build_remained_con(solution, num, build_flag = False):
    ...
    # print("best_edge_group_num: ", len(solution))
    # print("visit_len: ", len(visit[visit == 1]))

    for i in range(len(solution)):
        gen_visit(solution[i].cid1, solution[i].cid2)
    ...
    for i in range(num-len(solution)):
        if build_flag:
            solution.append(remain_con[i])
        gen_visit(remain_con[i].cid1, remain_con[i].cid2)

    # print("total_value:", cal_whole_value(solution, choose))
...
def build_lp_data_static():
    ...
    for i in range(len(city)):
        for j in range(i+1, len(city)):
            v_node.append(pl.LpVariable('A(%d)' % (len(v_node))))
            t_node.append(pl.LpVariable('B(%d)' % (len(t_node)), lowBound=0))
            v_node_map[i][j] = len(v_node) - 1
            v_node_map[j][i] = len(v_node) - 1

def build_lp_data_dynamic():
    # 2、边num分配
    global visit
    global v_edge_map
    global v_edge
    ...
    for i in range(len(city)):
        for j in range(i+1, len(city)):
            if visit[i][j] == 1:
                ...
                tmp_v_edge = [pl.LpVariable('V(%d,%d)' % (len(v_edge), k1), lowBound=-max_volume, upBound=max_volume)
                              for k1 in range(len(v_node))]
                tmp_t_edge = [pl.LpVariable('U(%d,%d)' % (len(t_edge), k2), lowBound=0, upBound=max_volume)
...
def build_constrain_static():
    global cons_static
    # 3.3 流量绝对值约束
    for i in range(len(v_node)):
        cons_static.append(v_node[i] <= t_node[i])
        cons_static.append(-1 * v_node[i] <= t_node[i])

def build_constrain_dynamic():
    cons = []
    # 3.1 点约束，流入流出的量相同
    # 比如是AB之间的传输
    for i in range(len(city)):
...
# 4、优化框架
def lp_frame(print_flag = False):
    global best_lp_value
    global lp_result
    # 1、数据
    build_lp_data_dynamic()
    cons = build_constrain_dynamic()
    ...
    for con in cons:
        prob += con
    for con in cons_static:
        prob += con
    status = prob.solve()
    lp_result = pl.value(target)
                ...
                if visit[i][j] == 1:
                    ...
                    for m in range(len(city)):
                        for n in range(m + 1, len(city)):
                            # 本线的流量变动了
                            if m == i and n == j:
                                if abs(pl.value(v_edge[v_edge_map[i][j]][v_node_map[m][n]])) <= edge_table[i][
                                    j].volume - threshold:
                                    if not find_flag:
                                    ...
                                    print(
                                        edge_table[i][j].volume - pl.value(v_edge[v_edge_map[i][j]][v_node_map[m][n]]),
                                        "mTb/s")
                                continue
                            # 运载的流量目标
...
def draw_result(solution=best_edge_group):
    ...
    for i in range(len(solution)):
        ...
        while True:
            ...
            tmp -= 1
            if tmp < 0:
                print("tmp_con.volume:", tmp_con.volume)
